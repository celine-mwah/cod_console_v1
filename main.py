...
class WAW_App_DPG:
    def __init__(self):
        ...
        self.current_active_tab = "Tweaks"
        self.is_animating_keyframes = False
        self.updating_fov = False
        self.debug_messages = []
    ...
    def _on_tab_changed(self, sender, app_data, user_data):
        self.current_active_tab = app_data
        self._add_debug_message(f"Switched to tab: {app_data}")

    def _get_current_active_tab(self):
        if dpg.does_item_exist("fog_color") and dpg.is_item_visible("fog_color"):
            return "Fog"
        elif dpg.does_item_exist("main_sun_color") and dpg.is_item_visible("main_sun_color"):
            return "Sun"
        elif dpg.does_item_exist("enhanced_keyframe_table") and dpg.is_item_visible("enhanced_keyframe_table"):
            return "Keyframing"
        elif dpg.does_item_exist("light_color") and dpg.is_item_visible("light_color"):
            return "Tweaks"
        else:
            return self.current_active_tab

    def _safe_update_fov_ui(self, new_fov, source="unknown"):
        if self.updating_fov:
            return

        self.updating_fov = True
        try:
            clamped_fov = max(5.0, min(165.0, new_fov))
            self.ui_vars["doubles"]["fov"] = clamped_fov

            fov_tags = ["fov", "kf_fov", "kf_modal_fov"]
            for tag in fov_tags:
                if dpg.does_item_exist(tag):
                    dpg.set_value(tag, clamped_fov)

            if source != "animation":
                self.memory_manager.execute_command(f"cg_fov {int(clamped_fov)}")
    ...
    def _main_float_prop_cb(self, sender, app_data, user_data):
        ...
        if prop_name == "fov":
            self._safe_update_fov_ui(app_data, source="main")
            if dpg.does_item_exist("kf_fov"):
                dpg.set_value("kf_fov", app_data)
        else:
    ...
    def _get_all_keyframable_values(self):
        return {
            ...
            "brightness": self.ui_vars["doubles"]["brightness"],
            "contrast": self.ui_vars["doubles"]["contrast"],
            "desaturation": self.ui_vars["doubles"]["desaturation"],
            "light_color": self.ui_vars["colors"]["light_color"],
            "dark_color": self.ui_vars["colors"]["dark_color"],
            "fog_start": self.ui_vars["doubles"]["fog_start"],
            "fog_color": self.ui_vars["colors"]["fog_color"],
            "fov": self.ui_vars["doubles"]["fov"],
        }
    ...
    def debug_current_keyframes(self):
        self._add_debug_message("=== CURRENT KEYFRAMES DEBUG ===")
        for i, kf in enumerate(self.current_keyframes):
            self._add_debug_message(f"Keyframe {i}: time={kf['time']}")
            for prop, value in kf.get('values', {}).items():
                if isinstance(value, list):
                    self._add_debug_message(f"  {prop}: [{value[0]:.2f}, {value[1]:.2f}, {value[2]:.2f}]")
                else:
                    self._add_debug_message(f"  {prop}: {value:.2f}")

    def transition_to_environment(self, preset_name, duration_ms):
        if self.is_transitioning:
            self._add_debug_message("Another transition is already in progress.", is_error=True)
            return
        end_preset = self.environment_presets.get(preset_name)
        if not end_preset:
            self._add_debug_message(f"Target preset '{preset_name}' not found for transition.", is_error=True)
            return
        self.stop_animation()
        self.stop_transition = False
    ...
    def _transition_worker(self, end_preset, duration_ms):
        ...
        if not self.stop_transition:
            self.apply_environment_preset(preset_name)
        self.is_transitioning = False
    ...
    def apply_environment_preset(self, preset_name):
        ...
        if not preset:
            self._add_debug_message(f"Environment preset '{preset_name}' not found", is_error=True)
            return
        ...
        if "flicker" in preset:
            ...
            if dpg.does_item_exist("flicker_preset_combo"):
                dpg.set_value("flicker_preset_combo", flicker_data.get("preset", "Pulse"))
            if dpg.does_item_exist("flicker_speed_ms"):
                dpg.set_value("flicker_speed_ms", flicker_data.get("speed", 500))
            if dpg.does_item_exist("flicker_use_easing"):
                dpg.set_value("flicker_use_easing", flicker_data.get("easing", True))
            self.start_flicker()
    ...
    def _start_legacy_animation(self):
        try:
            ...
            if dpg.get_value("main_enable_flicker_along_path"):
                self.start_flicker()

            anim_type = dpg.get_value("main_anim_type_selector")

            if anim_type == "Linear":
                cx, cy = self.ui_vars["doubles"]["sun_direction_x"], self.ui_vars["doubles"]["sun_direction_y"]
                tx, ty = dpg.get_value("main_anim_target_x"), dpg.get_value("main_anim_target_y")
                dur = float(dpg.get_value("main_anim_duration"))
                fps = int(dpg.get_value("main_anim_fps"))
                ease = dpg.get_value("main_anim_easing")
                reset = dpg.get_value("main_anim_reset_on_finish")
                ...
                cx = dpg.get_value("main_orbital_center_x")
                cy = dpg.get_value("main_orbital_center_y")
                rx = dpg.get_value("main_orbital_radius_x")
                ry = dpg.get_value("main_orbital_radius_y")
                rev_dur = float(dpg.get_value("main_orbital_rev_duration"))
                direction = dpg.get_value("main_orbital_direction")
    ...
    def apply_anim_preset(self, sender, app_data):
        ...
        if preset_data is None:
            preset_data = self.preset_manager.get_presets("animation").get(preset_name)
    ...
    def _add_debug_message(self, message, is_error=False):
        ...
        if len(self.debug_messages) > self.max_debug_lines:
            self.debug_messages = self.debug_messages[-self.max_debug_lines:]
    ...
    def _update_debug_display(self):
        ...
        if dpg.does_item_exist("debug_window"):
            dpg.set_y_scroll("debug_window", dpg.get_y_scroll_max("debug_window"))

    def _clear_debug_console(self):
        self.debug_messages = []
        if dpg.does_item_exist("debug_text"):
            dpg.set_value("debug_text", "Debug console cleared.")
    ...
    def _apply_config_to_ui(self, config_data):
        ...
        for category, values in ui_data.items():
            if category in self.ui_vars:
                self.ui_vars[category].update(values)
        self.sun_animator.update_all_properties(self._get_all_keyframable_values())
        for key, value in self.ui_vars["booleans"].items():
            if dpg.does_item_exist(f"{key}_bool"):
                dpg.set_value(f"{key}_bool", value)
        ...
        self.hotkey_manager.reregister_all_hotkeys(self.hotkeys)
        if dpg.does_item_exist("hotkey_table"):
    ...
    def _scan_for_demos(self):
        if not os.path.exists(self.demo_path):
            self._add_debug_message(f"Demo directory not found at {self.demo_path}", is_error=True)
            return []
        try:
    ...
    def _toggle_hud_cb(self, sender, app_data, user_data):
        self.hud_visible = not self.hud_visible
        value = 1 if self.hud_visible else 0
        self.memory_manager.execute_command(f"cg_draw2d {value}")
        self._add_debug_message(f"HUD {'enabled' if self.hud_visible else 'disabled'}.")
    ...
    def _float_prop_cb(self, sender, app_data, user_data):
        ...
        if prop_name == "fov":
            self._safe_update_fov_ui(app_data, source="manual")
        else:
            self.ui_vars["doubles"][prop_name] = app_data
            self.memory_manager.execute_command(f"{command} {app_data:.2f}")
    ...
    def update_hotkey_tab_ui(self):
        if dpg.does_item_exist("hotkey_table"):
            dpg.delete_item("hotkey_table", children_only=True)
        ...
        for action_name, description in self.hotkey_manager.bindable_actions.items():
            with dpg.table_row(parent="hotkey_table"):
                dpg.add_text(description)
                tag = f"hotkey_input_{action_name}"
                dpg.add_input_text(tag=tag)
                dpg.set_value(tag, self.hotkeys.get(action_name, ""))
        for preset_type, label in [("animation", "Anim"), ("flicker", "Flicker")]:
            for name in self.preset_manager.get_presets(preset_type):
                with dpg.table_row(parent="hotkey_table"):
                    dpg.add_text(f"({label} Preset) {name}")
                    tag = f"hotkey_input_{preset_type}_preset:{name}"
                    dpg.add_input_text(tag=tag)
                    dpg.set_value(tag, self.hotkeys.get(f"{preset_type}_preset:{name}", ""))
        for action_name in self.preset_manager.get_presets("actions"):
            with dpg.table_row(parent="hotkey_table"):
                dpg.add_text(f"(Custom Action) {action_name}")
                tag = f"hotkey_input_custom:{action_name}"
                dpg.add_input_text(tag=tag)
                dpg.set_value(tag, self.hotkeys.get(f"custom:{action_name}", ""))
    ...
    def create_ui(self):
        with dpg.window(tag="Primary Window", autosize=False, width=640, height=850, no_collapse=False):
            ...
            with dpg.group(horizontal=True):
                dpg.add_input_text(label="", tag="console_input", width=200, on_enter=True,
            ...
            with dpg.group(horizontal=True):
                def timescale_callback(s, d, u):
                    try:
                        val = float(d)
                        self.memory_manager.execute_command(f"timescale {val}")
                    except (ValueError, TypeError):
                        self._add_debug_message(f"Invalid timescale value: {d}", is_error=True)

                dpg.add_input_text(label="Timescale", tag="timescale_input", width=50, on_enter=True,
                                   callback=timescale_callback, default_value="1.0", scientific=True)
                dpg.add_button(label="Set", callback=lambda: timescale_callback(None, dpg.get_value("timescale_input"), None))
            ...
            with dpg.tab_bar(callback=self._on_tab_changed):
                with dpg.tab(label="Tweaks"):
                    ...
                    with dpg.group(horizontal=True):
                        ...
                        dpg.add_checkbox(label="Toggle HUD", callback=self._toggle_hud_cb)
                    dpg.add_separator()

                    dpg.add_slider_float(label="FOV", tag="fov", default_value=self.ui_vars["doubles"]["fov"],
                                         min_value=5, max_value=165, callback=self._main_float_prop_cb,
                                         user_data=["fov", "cg_fov"])

                    dpg.add_slider_float(label="Brightness", tag="brightness",
                                         default_value=self.ui_vars["doubles"]["brightness"], min_value=-1.0,
                                         max_value=1.0, callback=self._main_float_prop_cb,
                    ...
                    dpg.add_slider_float(label="Contrast", tag="contrast",
                                         ...
                                         max_value=4.0, callback=self._main_float_prop_cb,
                    ...
                    with dpg.group(horizontal=True):
                        def execute_sequence():
                            ...
                            if steps:
                                self.sequence_builder.execute_sequence(name, steps, multiplier)
                ...
                with dpg.tab(label="Fog"):
                    ...
                    dpg.add_slider_float(label="Fog Density", tag="fog_start",
                                         default_value=self.ui_vars["doubles"]["fog_start"], min_value=1,
                                         max_value=9999, callback=self._main_float_prop_cb,
                    ...
                    dpg.add_color_picker(tag="fog_color", label="Fog Color", no_side_preview=True, alpha_bar=False,
                                         ...
                                         callback=self._main_color_prop_cb, user_data=["fog_color", "mvm_fog_color"])
                ...
                with dpg.tab(label="Sun"):
                    ...
                    dpg.add_text(
                        "Note: Animation does not work with cl_avidemo -- Please record with OBS and timescale",
                        color=(255, 0, 0))
                ...
                with dpg.tab(label="Keyframing"):
                    dpg.add_text("KEYFRAMES DO NOT WORK WITH CL_AVIDEMO -- PLEASE RECORD WITH OBS AND TIMESCALE",
                                 color=(255, 0, 0))
                    ...
                    dpg.add_button(label="Debug Current Keyframes", callback=self.debug_current_keyframes, width=200)
                ...
                with dpg.tab(label="Binds"):
                    ...
                    def update_actions_list():
                        if dpg.does_item_exist("custom_actions_list"):
                            dpg.delete_item("custom_actions_list", children_only=True)
                    ...
                    def save_hotkeys_callback():
                        new_config = {}
                        for action in self.hotkey_manager.bindable_actions:
                            new_config[action] = dpg.get_value(f"hotkey_input_{action}").lower()
                        for preset_type in ["animation", "flicker"]:
                            for name in self.preset_manager.get_presets(preset_type):
                                new_config[f"{preset_type}_preset:{name}"] = dpg.get_value(
                                    f"hotkey_input_{preset_type}_preset:{name}").lower()
                        for name in self.preset_manager.get_presets("actions"):
                            new_config[f"custom:{name}"] = dpg.get_value(f"hotkey_input_custom:{name}").lower()
                ...
                with dpg.tab(label="Config"):
                    def refresh_list():
                        ...
                        for f in list_config_files():
                            dpg.add_button(label=f, width=-1, callback=load, user_data=f, parent="config_list_group")
                ...
                with dpg.tab(label="Debug"):
                    with dpg.group(horizontal=True):
                        ...
                        dpg.add_button(label="Clear", callback=self._clear_debug_console)
                    with dpg.child_window(tag="debug_window", width=-1, height=-1, horizontal_scrollbar=True):
                        dpg.add_text("Debug console loaded", tag="debug_text", wrap=0)
                dpg.add_tab(label="|")
                ...
                with dpg.tab(label="About"):
                    ...
                    if dpg.does_item_exist("about_texture"):
                        dpg.add_image("about_texture")
        ...
        with dpg.window(label="Action Editor", modal=True, show=False, tag="action_editor_window", width=400,
            ...
            def save_action():
                ...
                if not name:
                    self._add_debug_message("Action name cannot be empty.", is_error=True)
                    return
                steps = []
